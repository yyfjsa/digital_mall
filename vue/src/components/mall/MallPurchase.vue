    ...
    <div class="product-box">
      <div class="nav-bar">
        <div v-if="productInfo" class="container">
          <el-image class="logo animate__animated animate__rubberBand" style="width: 65px;height: 65px;margin-top: 8px;margin-right: 10px;" :src="productInfo.productUrl"></el-image>
      ...
      <div class="product-info-content container">
        ...
        <div class="product-con">
          ...
          <div class="address-box container">
            ...
            <div class="address-alter" @click="change1">修改</div>
          </div>
    ...
    data() {
      ...
      let validateContactWay = (rule, value, callback) => {
        ...
        if (!/^[0-9\-]+$/.test(trimmedValue)) {
          return callback(new Error('请输入数字和 -'));
        }
        const regex = /^((0\d{2,3}-\d{7,8})|(1[3-9]\d{9}))$/;
        if (!regex.test(trimmedValue)) {
          return callback(new Error('请填写正确的手机号或固话'));
        }
        callback();
      };
            ...
            if (valid) {
              ...
              this.$http.post('/order/add',this.$qs.stringify(this.order,{skipNulls: true})).then((rep)=>{
                ...
                if(rep.data.code===200){
                  ...
                  let orderName = '商城-'+this.productInfo.productType+'-'+this.productInfo.productName+'支付订单';
