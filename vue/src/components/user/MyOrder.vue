<template>
  <div class="my-order">
            ...
            <el-table-column align="center" label="操作">
              <template slot-scope="scope">
                ...
                <el-button
                  ...
                  v-if="scope.row.orderState==='退货完成' || scope.row.orderState==='已评价'">隐藏订单</el-button>
              </template>
            </el-table-column>
          </el-table>
        </el-tab-pane>
    ...
    <el-dialog title="客服" :visible.sync="serviceVisible" width="30%" style="padding: 0 20px 0">
      <!-- 使用相对路径或 @ 符号引入本地图片 -->
      <el-image :src="require('@/assets/img/weixin.jpg')"></el-image>
    ...
    methods:{
      ...
      payment(orderNo,orderTime,productName,payPrice,productType){
        if(this.isTimeOut(orderTime)){
          ...
          let orderName = '商城-'+productType+'-'+productName+'支付订单';
      ...
      delOrder(orderId){
        if(orderId){
          let loading = this.$loading({lock: true, text: "数据提交中",background:"rgba(255,255,255,0.2)"});
          this.$http.post('/order/delete?orderId='+orderId).then((rep)=>{
            loading.close();
            if(rep.data.code===200){
              this.reqInfo();
            }else{
