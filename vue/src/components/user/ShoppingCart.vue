<template>
  <div class="shopping-cart">
    ...
    <div class="product-box">
      <div class="nav-bar">
        <div class="container">
          <router-link to="/MallHome" style="text-decoration: none" title="官网">
      ...
      <div class="cart-info" v-if="tableData.length!==0">
        ...
        <div class="cart-bar">
          ...
          <div class="section-right">
            <span class="total">
            合计：
            <span style="font-size: 30px">{{ summation.toFixed(2) }}</span>
            元
          </span>
            <el-tooltip content="请勾选需要结算的数码" v-if="selectedData" :disabled="selectedData.length!==0" placement="top" effect="light">
              <div class="button">
...
export default {
  ...
  methods:{
    ...
    createOrder(){
      ...
      if(!this.baseInfo.acceptAddress){
        this.$message({
          type:'error',
          message:'填写地址'
        })
      ...
      this.$http.post('/order/cartOrder',this.$qs.stringify(send)).then((rep)=>{
        ...
        if(rep.data.code===200){
          ...
          this.$alert(msg, '下单成功', {
            ...
            type: 'success',
            callback: () => {
              let orderName = this.getCurrentDate()+'商城-'+this.baseInfo.userName+'购物车支付订单';
    ...
    calculationPrice(index) {
      // 计算时将所有数值转为整数（分），避免浮动误差
      const productPriceInCents = Math.round(this.tableData[index].productPrice * 100); // 将价格转换为“分”
      const discountInCents = Math.round(this.tableData[index].discount * 100); // 将折扣转换为整数，避免浮动误差
      const payAmountInCents = Math.round(this.tableData[index].payAmount * 100); // 将支付数量转换为“分”

      // 计算 totalPrice，以“分”为单位，再将结果转为“元”
      let calculatedPriceInCents = payAmountInCents * (productPriceInCents * discountInCents / 1000000); // 除以1000000是为了恢复元单位

      // 保留两位小数
      this.tableData[index].totalPrice = Math.round(calculatedPriceInCents) / 100;

      // 重置合计
      this.summation = 0;

      // 检查 selectedData 是否有数据
      if (this.selectedData && this.selectedData.length > 0) {
        for (let i = 0; i < this.selectedData.length; i++) {
          // 确保 totalPrice 被解析为浮动数，并且四舍五入到小数点后两位
          this.summation += parseFloat(this.selectedData[i].totalPrice) || 0;
        }

        // 使用 Math.round 进行四舍五入并确保合计保留两位小数
        this.summation = Math.round(this.summation * 100) / 100; // 除以100后再乘以100确保保留两位小数
      } else {
        console.warn("selectedData is empty or undefined");
      }
    },
    deleteProduct(index){
      this.$confirm('此数码将从购物车中移除?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
    ...
    reqInfo() {
      ...
      this.$http.post('/shoppingCart/findAll?account=' + this.$store.state.user['accountNumber'])
        .then((rep) => {
          loading.close();
          ...
          if (rep.data.code === 200) {
            ...
            for (let i = 0, len = this.tableData.length; i < len; i++) {
              if (isVip) {
                if (this.tableData[i].productPrice >= 5000) {
                  this.tableData[i].discount = 90;
                } else if (this.tableData[i].productPrice >= 1000) {
              ...
              // 使用 discount 计算 totalPrice
              this.tableData[i].totalPrice = (this.tableData[i].payAmount * (this.tableData[i].productPrice * this.tableData[i].discount / 100)).toFixed(2);
            }
          }
        })
        .catch((err) => {
