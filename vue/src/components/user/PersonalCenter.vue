          ...
          <el-image
            ...
            <div slot="error" style="height: 100%"><!--加载失败或未设置图片后显示的图片内容-->
              <el-image :lazy="true" style="width: 100%; height: 100%" fit="none"
                        src="https://jsazxxz1.oss-cn-beijing.aliyuncs.com/backiee-84337.jpg">
              </el-image>
            </div>
          </el-image>
...
export default {
  ...
  //   if (this.isVip) {
  //     this.$http.post(this.$store.state.user['accountNumber']).then((rep) => {
  //       if (rep.data.code === 200) {
  //         this.vip = rep.data.data;
  //       }
  //     }).catch((err) => {
  ...
  methods: {
    ...
    editPhoneCode() {
      //匹配手机号正则表达式
      let regex = /^((0\d{2,3}-\d{7,8})|(1[3-9]\d{9}))$/;
